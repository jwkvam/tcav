...
def load_image_from_file(filename, shape):
  ...
  """
    if not os.path.exists(filename):
        ...
        return None
    try:
        img = np.array(PIL.Image.open(tf.gfile.Open(filename)).resize(shape, PIL.Image.BILINEAR))
...
def load_images_from_files(
  ...
  """
    ...
    if run_parallel:
        ...
        imgs = pool.map(
            lambda filename: load_image_from_file(filename, shape), filenames[:max_imgs]
        )
        if return_filenames:
...
def process_and_load_activations(
    model, bottleneck_names, concepts, source_dir, acts_dir, acts=None, max_images=500
):
  ...
  """
    ...
    for concept in concepts:
        ...
        for bottleneck_name in bottleneck_names:
            acts_path = os.path.join(acts_dir, 'acts_{}_{}'.format(concept, bottleneck_name))
            ...
            if bottleneck_name not in acts[concept].keys():
                with open(acts_path) as f:
                    ...
                    tf.logging.info(
                        'Loaded {} shape {}'.format(acts_path, acts[concept][bottleneck_name].shape)
                    )
            else:
                tf.logging.info(
                    '%s, %s already exists in acts. Skipping...' % (concept, bottleneck_name)
                )
